...
# Get step 4705 payload to extract the forms
cursor.execute("SELECT step_payload FROM steps WHERE idx=4705")
row_4705 = cursor.fetchone()
data_4705 = row_4705[0]
conn.close()
...
parse_message(data_4705)

if not code_4705:
    print("Failed to find step 4705 code!")
    sys.exit(1)

print("Found step 4705 code.")

# Extract state2_inv (Interview management form)
colloquio_start = code_4705.find("{/* TAB 3: INSERISCI COLLOQUIO */}")
check_inv = "selectedInterviewForManagement ? ("
check_inv_idx = code_4705.find(check_inv, colloquio_start)
else_inv = ") : ("
else_inv_idx = code_4705.find(else_inv, check_inv_idx)

if check_inv_idx != -1 and else_inv_idx != -1:
    state2_inv = code_4705[check_inv_idx + len(check_inv) : else_inv_idx].strip()
    print("Extracted state2_inv from 4705! Size:", len(state2_inv))
else:
    print("Failed to extract state2_inv!")
    sys.exit(1)

# Extract state2_tr (Trial management form)
prova_start = code_4705.find("{/* TAB 4: PROVE */}")
check_tr = "selectedTrialForManagement ? ("
check_tr_idx = code_4705.find(check_tr, prova_start)
else_tr = ") : ("
else_tr_idx = code_4705.find(else_tr, check_tr_idx)

if check_tr_idx != -1 and else_tr_idx != -1:
    state2_tr = code_4705[check_tr_idx + len(check_tr) : else_tr_idx].strip()
    print("Extracted state2_tr from 4705! Size:", len(state2_tr))
else:
    print("Failed to extract state2_tr!")
    sys.exit(1)

# Read the current file on disk (which is step 5298 with modal containers)
file_path = 'gestionale_ricerca_locale/frontend/src/App.jsx'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace empty bodies in App.jsx
# Interview modal body replacement target
...
# Trial modal body replacement target
tr_body_target = """            <div className="modal-body" style={{ flexGrow: 1, overflowY: 'auto', padding: '24px' }}>
              
            </div>"""

# Since both targets are identical, we must replace them precisely using find/slice or specific markers
# Let's find the exact interview modal section
...
if interview_idx != -1 and trial_idx != -1:
    ...
    trial_idx = content.find(trial_modal_marker)
    tr_body_start = content.find(tr_body_target, trial_idx)
    if tr_body_start != -1:
        tr_body_replacement = """            <div className="modal-body" style={{ flexGrow: 1, overflowY: 'auto', padding: '24px' }}>
              """ + state2_tr + """
            </div>"""
        content = content[:tr_body_start] + tr_body_replacement + content[tr_body_start + len(tr_body_target):]
