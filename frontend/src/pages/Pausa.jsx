...
export default function Pausa({ handleApprovalAction, setSelectedRicercaId }) {
  ...
  return (
    <div>
      <h2 style={{ fontSize: '16px', marginBottom: '16px', fontWeight: 700 }}>Mandati Messi in Pausa</h2>
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {ricerche.filter(r => r.stato_approvazione_tl === 'In Pausa').map(r => (
              <tr key={r.id}>
            ...
            {ricerche.filter(r => r.stato_approvazione_tl === 'In Pausa').length === 0 && (
              <tr>
                <td colSpan="6" style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>Nessun mandato in pausa.</td>
              </tr>
            )}
          </tbody>
        </table>
